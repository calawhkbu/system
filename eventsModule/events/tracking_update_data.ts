    ...
    await Promise.all(trackingReferenceList.map(async trackingReference => {
      ...
      const resultList = bookingList.map(booking => {
        ...
        for (const iterator of alertMap) {
          ...
          const trackingTime = (tracking[iterator.trackingVariable] ? moment(tracking[iterator.trackingVariable]).utc().toDate() : undefined) as Date
          ...
          if (trackingTime) {
            ...
            if (bookingTime) {
              ...
              const minTime = moment(bookingTime).utc().subtract(deplayAlertTimeRange[booking.moduleTypeCode].value, deplayAlertTimeRange[booking.moduleTypeCode].unit)
              const maxTime = moment(bookingTime).utc().add(deplayAlertTimeRange[booking.moduleTypeCode].value, deplayAlertTimeRange[booking.moduleTypeCode].unit)
