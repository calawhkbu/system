...
const app = {
  ...
  ) => `${constants.url}/reports/uber/count`,
  requestHandler: async(
    ...
    const customerPartyList = user.parties.map((x: any) => partyList.find((y: any) => y.id === x.id))

    customerPartyList.forEach((customerParty: any) => {
    ...
    for (const [partyType, outboundPartyType] of partymap) {
      if (subqueries[partyType]) {
        const localPartyList = subqueries[partyType].value.map((partyId: any) =>
          partyList.find((y: any) => y.id === partyId)
        )
        filter[outboundPartyType] = {}
        filter[outboundPartyType][outboundPartyType] = localPartyList
          .map((x: any) => x.thirdPartyCode.old360)
          .filter((x: any) => x)
  ...
  responseHandler: (
    ...
    return {
      responseBody: [{ count: responseBody.count }],
      responseOptions,
    }
  },
}
