...
const app = {
  ...
  getUrl: (headers: any, body: any, constants: { [key: string]: any }) => `${constants.url}/reports/shipment-status`,
  requestHandler: async(
    ...
    let { fields = [], sorting, groupBy } = query
    ...
    fields = fields.map((fieldName: any) => helper.convertToExternal(fieldName, fieldNameMap))

    if (groupBy && groupBy.length) {
      groupBy = groupBy.map((fieldName: any) => helper.convertToExternal(fieldName, fieldNameMap))
    }
    if (sorting) {
    ...
    if (statusList) {
      const statusIdMap = (constants.statusIdMap = helper.create2WayMap(
        constants.statusIdMap,
        'external',
        'internal'
      ))
      filter.status = {
        list: statusList.value.reduce((resultList: any, singleStatus: any) => {
          resultList.push(helper.convertToExternal(singleStatus, statusIdMap))
          return resultList
        }, [])
      }
    }
    ...
    const customerParyIdList = user.parties.map(x => x.id)
    ...
    for (const [partyType, value] of partymap) {
    ...
    customerPartyList.forEach(customerParty => {
      if (customerParty.isBranch) {
        ...
        partySecurityList.push(customerParty.thirdPartyCode.erp)
      }
    })
    ...
    if (partySecurityList.filter(x => x).length) {
      ...
      filter.partySecurity = { code: partySecurityList.filter(x => x) }
    }
  ...
  responseHandler: (
    ...
    // converting the status Name into our status Code

    const statusMap = (constants.statusMap = helper.create2WayMap(
      constants.statusMap,
      'external',
      'internal'
    ))
    ...
    data.map(statusData => {
      statusData.status = helper.convertToInternal(statusData.status, statusMap)
    })
