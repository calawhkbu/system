...
import { expandGroupEntity, groupByEntityList,summaryVariableList,summaryVariableListBooking,groupByEntityListBooking } from 'utils/card'
  ...
  jqls: [
    {
      ...
      prepareParams(params, prevResult: Result): IQueryParams {
        ...
        if (!subqueries.entityType) throw ERROR.MISSING_ENTITY_TYPE()

        if (subqueries.entityType === 'booking'){
          let groupByEntityList = groupByEntityListBooking
          let summaryVariableList = summaryVariableListBooking 
        }
