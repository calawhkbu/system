    ...
    $select: [
      ...
      new ResultColumn(
        new FunctionExpression(
            ...
            new Query({
              ...
              $where: new AndExpressions([
                new BinaryExpression(
                  new ColumnExpression('raw', 'controllingCustomerPartyCode'),
                  '=',
          ...
          new FunctionExpression(
            'IF',
            new BinaryExpression(new ColumnExpression('raw', 'nominatedType'), '=', 'F'),
            'F',
            'R'
          ),
...
function prepareInitTable(
  fromTableName: string,
  group: { majorGroup: string; minorGroup: { columnName: string; option: string[] }[] },
  summary: { columnName: string; summaryBy: string }[],
  name: string
): CreateTableJQL {
  const groupByColumnList = group.minorGroup
    .map(({ columnName }) => columnName)
    .concat(group.majorGroup)

  const groupByList = groupByColumnList
    .map((columnName: string) => new ColumnExpression(fromTableName, columnName))
    .concat(new ColumnExpression(fromTableName, 'jobMonth'))
  ...
  summary.forEach(({ columnName, summaryBy }) => {
    selectList.push(
      new ResultColumn(
        new FunctionExpression(
          'IFNULL',
          new FunctionExpression(summaryBy, new ColumnExpression(fromTableName, columnName)),
          0
        ),
        `${columnName}Total`
      )
...
function prepareMonthTable(
  fromTableName: string,
  group: { majorGroup: string; minorGroup: { columnName: string; option: string[] }[] },
  summary: { columnName: string; summaryBy: string }[],
  name: string
) {
  const fullGroupByColumnList = group.minorGroup
    .map(({ columnName }) => columnName)
    .concat(group.majorGroup)

  const queryObject = {
    $select: fullGroupByColumnList.map(
      column => new ResultColumn(new ColumnExpression(fromTableName, column), column)
    ),

    $from: fromTableName,
    $group: new GroupBy(
      fullGroupByColumnList.map(columnName => new ColumnExpression(fromTableName, columnName))
    ),
  }
  ...
  function getSelectList(
    ...
    const findSummaryCondition = (columnName, optionValue) => {
      ...
      reminaingMinorGroup.forEach(({ columnName, option }) => {
        result.push(
          new InExpression(new ColumnExpression(fromTableName, columnName), false, option)
        )
      })

      result.push(
        new BinaryExpression(new ColumnExpression(fromTableName, columnName), '=', optionValue)
      )
            ...
            new ResultColumn(
              new FunctionExpression(
                ...
                new FunctionExpression(
                  ...
                  new FunctionExpression(
                    'IF',
                    new AndExpressions(currentConditionList),
                    new ColumnExpression(fromTableName, `${columnName}${monthkey}Total`),
                    0
                  )
                ),
                0
              ),
              monthSubTotalColumnName
            )
          )
        }
            ...
            new FunctionExpression(
              ...
              new FunctionExpression(
                ...
                new FunctionExpression(
                  'IF',
                  new AndExpressions(currentConditionList),
                  new ColumnExpression(fromTableName, `${columnName}Total`),
                  0
                )
              ),
              0
