...
export default class EdiParser850 extends BaseEdiParser {
    ...
    async import(base64EdiContent: string, partyGroupCode: string): Promise<any> {

        console.log(`import type  : ${this.type}`)

        const result = await this.callImportOutbound(base64EdiContent, partyGroupCode)

        const products = []

        const repo = 'customer-' + partyGroupCode

        const response = await (this.allService.OutboundService as OutboundService).send(
          repo,
          'edi-import',
          {},
          {
