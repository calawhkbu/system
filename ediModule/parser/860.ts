    ...
    if (Array.isArray(ediJson['ST'])) {
      for (const ST of ediJson['ST']) {
        ...
        if (_.get(ST, 'BCH.poChangeRequestDate')) {
          po.flexData.data['changeRequestDate'] = moment
            .utc(_.get(ST, 'BCH.poChangeRequestDate'))
            .toDate()
        ...
        if (Array.isArray(ST['N1'])) {
          for (const N1 of ST['N1']) {
            ...
            if (_.get(N1, 'organizationIdentifier') === 'Ship From') {
              ...
              po.flexData.data[`${name}PartyAddress1`] = _.get(
                N1,
        ...
        if (Array.isArray(ST['POC'])) {
          for (const POC of ST['POC']) {
            ...
            product['sea'] = null
            product['style'] = null
            product['styleDesc'] = _.get(POC, 'PID.description', '')
              .substr(0, 20)
              .replace(/^[ ]+|[ ]+$/g, '')
            product['piece'] = null
            product['pieceDesc'] = _.get(POC, 'PID.description', '')
              .substr(40, 20)
              .replace(/^[ ]+|[ ]+$/g, '')
            product['color'] = null
            product['colorDesc'] = _.get(POC, 'PID.description', '')
              .substr(20, 20)
              .replace(/^[ ]+|[ ]+$/g, '')
            product['pack'] = null
            product['packing'] = ''
