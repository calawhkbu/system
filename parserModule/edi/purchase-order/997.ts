...
export default class EdiParser997 extends BaseEdiParser {
  ...
  async export(entityJSON: any[]| (any[])[]): Promise<any> {
    ...
    ISA.elementList.push('00', '\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0', '00',
     '\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0\xa0', _.get(entityJSON, 'ISAReceiverQl'), _.get(entityJSON, 'ISAReceiverId'), _.get(entityJSON, 'ISASenderIdQl'), _.get(entityJSON, 'ISASenderId'), moment(currantDate).format('YYMMDD'), moment(currantDate).format('HHmm'), 'U', '00403', _.get(entityJSON, 'interchangeControlNumber'), '0', testMapper[testIndicator], '>')
    data.push(ISA)
    const GS: JSONObject = {
        segment: 'GS',
        elementList: []
    ...
    GS.elementList.push('FA', _.get(entityJSON, 'receiverId'), (senderIdMapper[applicationsenderId] || applicationsenderId), moment(currantDate).format('YYYYMMDD'), moment(currantDate).format('HHmm'), _.get(entityJSON, 'dataInterchangeControlNumber'), 'X', _.get(entityJSON, 'versionId'))
    data.push(GS)
    ...
    if (_.get(entityJSON, 'poList').length)
    {
      ...
      const errorList = _.get(entityJSON, 'errors').filter(x => x.category === 'segmentError' || x.category === 'elementError' || x.category === 'transactionSetSyntaxError')
  ...
  async getLoopObject(loopObjectList, entityJSON, noOfSt)
  {
    ...
    {
      const segmentErrorList: any [] = errorList.filter(x => x.category === 'segmentError' && x.mainHeadIndex === i)
      const elementErrorList: any [] = errorList.filter(x => x.category === 'elementError' && x.mainHeadIndex === i)
      const transactionSetSyntaxErrorList: any [] = errorList.filter(x => x.category === 'transactionSetSyntaxError' && x.mainHeadIndex === i)
      ...
      if (segmentErrorList.length || elementErrorList.length)
      {
        ...
        for (const segmentError of segmentErrorList)
        {
          if (segmentErrorMapper[segmentError.errorType])
          {
            ...
            AK3.elementList.push(segmentError.errorID, segmentError.afterMainHeadLocation.toString() , loopIndex.toString(), segmentErrorMapper[segmentError.errorType])
        ...
        for (const elementError of elementErrorList)
        {
          ...
          AK4.elementList.push(elementError.segmentPosition.toString(), elementError.errorIdex.toString() , loopIndex.toString(), elementErrorMapper[elementError.errorType], elementError.element)
        ...
        if (transactionSetSyntaxErrorList.length)
        {
          for (const transactionSetSyntaxError of transactionSetSyntaxErrorList)
          {
            const AK5: JSONObject = {
              ...
              AK5.elementList.push(transactionSetSyntaxErrorMapper[transactionSetSyntaxError.errorType])
              loopObjectList.push(AK5)
          }
        }
      }
