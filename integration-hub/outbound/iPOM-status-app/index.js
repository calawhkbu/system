function iPOMStatusHandler() {
  this.handle = function(appId, params, helper) {
    ...
    if (status) {
      ...
      return helper.persistence.models.booking
        .findOne({ where: { id: status.bookingId } })
        .then(booking => {
          if (booking) {
            console.log(`[iPOM-status-app] have booking`)
            return helper.persistence.models.customer
              .findOne({ where: { id: booking.customerId } })
              .then(customer => {
                if (
                  customer &&
                  customer.configuration &&
                  customer.configuration.webService.purchaseOrder &&
                  customer.configuration.webService.purchaseOrder.statusApi
                ) {
                    ...
                    helper.emailer.sendFreeMail(
                      {
                        ...
                        html: `<p>Payload:</p><br/><p>${JSON.stringify(
                          payload
                        )}</p><p>Error:</p><br/><p>${JSON.stringify(e.message)}</p><p>${
                          e.stack
                        }</p>`,
                      },
                      {}
