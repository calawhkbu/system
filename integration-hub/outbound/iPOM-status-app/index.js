function iPOMStatusHandler () {
  this.handle = function (appId, params, helper) {
    ...
    if (status) {
      ...
      return helper.persistence.models.booking.findOne({ where: { id: status.bookingId } })
        .then(booking => {
          if (booking) {
            console.log(`[iPOM-status-app] have booking`)
            return helper.persistence.models.customer.findOne({ where: { id: booking.customerId } })
              .then(customer => {
