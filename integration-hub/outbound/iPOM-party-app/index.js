function iPOMPartyHandler() {
  this.handle = function(appId, params, helper) {
    ...
    helper.persistence.models.customer
      .findOne({ where: { id: party.customerId } })
      .then(customer => {
        if (
          customer &&
          customer.configuration &&
          customer.configuration.webService.purchaseOrder &&
          customer.configuration.webService.purchaseOrder.partyApi
        ) {
                  ...
                  helper.emailer.sendFreeMail(
                    {
                      ...
                      html: `<p>Party:</p><br/><p>${JSON.stringify(
                        payload
                      )}</p><p>Data Returned:</p><br/><p>${postData}</p>`,
                    },
                    { party }
                  )
            ...
            helper.emailer.sendFreeMail(
              {
                ...
                html: `<p>Party:</p><br/><p>${JSON.stringify(
                  payload
                )}</p><p>Error:</p><br/><p>${JSON.stringify(e.message)}</p><p>${e.stack}</p>`,
              },
              { party }
            )
            helper.saveLog(
