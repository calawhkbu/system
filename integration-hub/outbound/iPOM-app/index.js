function iPOMHandler() {
  this.handle = function(appId, params, helper) {
    ...
    helper.persistence.models.customer
      .findOne({ where: { id: purchaseOrder.customerId } })
      .then(customer => {
        if (
          customer &&
                  ...
                  helper.emailer.sendFreeMail(
                    {
                      ...
                      html: `<p>PO:</p><br/><p>${JSON.stringify(
                        reqPayLoad
                      )}</p><p>Data Return:</p><br/><p>${JSON.stringify(postData)}</p>`,
                    },
            ...
            helper.emailer.sendFreeMail(
              {
                ...
                html: `<p>PO:</p><br/><p>${reqPayLoad}</p><p>Error:</p><br/><p>${JSON.stringify(
                  e.message
                )}</p><p>${e.stack}</p>`,
              },
              { purchaseOrder: purchaseOrder }
